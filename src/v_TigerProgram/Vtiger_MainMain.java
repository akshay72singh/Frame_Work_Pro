...
//        
        ...
        for (int j = 0; j < marketingClick.size(); j++) {
        	...
        	for(int ie = 0 ; ie<submodule.size();ie++) {
        		submodule = vtiger.findElements(By.xpath("//td[contains(@class,'SelTab')]"));                         // * click on marketing and leads
       ...
       List<WebElement>standerd_CheckBox= vtiger.findElements(By.xpath("//table[@id='modmgr_standard']//tr//a/img[contains(@alt,'able')]"));
		for (int iw = 0; iw < standerd_CheckBox.size(); iw++) {
			standerd_CheckBox= vtiger.findElements(By.xpath("//table[@id='modmgr_standard']//tr//a/img[contains(@alt,'able')]"));
		...
		Thread.sleep(5000);
		vtiger.findElement(By.xpath("//img[@title=\"Create Sales Order...\"]")).findElement(By.xpath("//img[@title='Show World Clock...']")).click();        	 //click on creat sales order in mouse over on sales
		...
		WebElement tagCloud = vtiger.findElement(By.xpath("//b[contains(text(),'Tag Cloud')]"));                         // Click and release tag cloud
		Thread.sleep(5000);
		moveTagCloud.dragAndDrop(TopPotentials, tagCloud).build().perform();

 		moveTagCloud.moveToElement(TopPotentials).clickAndHold().moveToElement(tagCloud).release().build().perform();
		...
		for (int ia = 0; ia < mouseoverAllLeads.size(); ia++) {
			...
			WebElement  mouseoverAllLeadsGet = mouseoverAllLeads.get(ia);                                                  //mouse over and click on all tab(link)
		...
		for (int j = 0; j < subModule.size(); j++) { 
			subModule = vtiger.findElements(By.xpath("//td[contains(@class,'SelTab')]"));                               // * mouse over and click on submodule
